...
func _ready() -> void:
	...
	if not level_row_scene:
		level_row_scene = load("uid://be50rwf3yw6y")
...
func _on_play_requested(level_info: LevelInformation) -> void:
	...
	if current_scene and current_scene.has_method("start_level"):
		current_scene.call_deferred("start_level", level_info.scene)
	else:
		get_tree().change_scene_to_packed(level_info.scene)
	queue_free()
...
func _on_preview_requested(level_info: LevelInformation) -> void:
	...
	if not preview_rect:
		return
	if not level_info or not level_info.preview_image:
		preview_rect.texture = preview_placeholder
		return
	preview_rect.texture = level_info.preview_image
