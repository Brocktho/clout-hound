...
@onready var title_label: Label = $UI/MainMenu/VBoxContainer/TitleLabel
@onready var title_particles: GPUParticles2D = $UI/MainMenu/VBoxContainer/TitleLabel/TitlePulseParticles
...
@export var title_pulse_bpm: float = 144.0
@export var title_pulse_scale: float = 1.08
@export var title_pulse_attack: float = 0.12
@export var title_pulse_release: float = 0.28
@export var title_pulse_particles_enabled: bool = true

var _rng := RandomNumberGenerator.new()
var _floater_toggle := false
var _dog_mesh: Mesh = preload("uid://bhjvh3vj2grbx")
var _board_mesh: Mesh = preload("uid://djvoga0acqib2")
...
var _title_pulse_running: bool = false
...
func _ready():
	...
	if not play_scene: play_scene = load("uid://gess2apgxsa3")
	if not settings_scene: settings_scene = load("uid://be50rwf3yw6y")
	if not credits_scene: credits_scene = load("uid://dp7n6x23g8dl4")
...
func start_level(scene: PackedScene) -> void:
	if not scene_host or not scene:
		return
	if _active_scene and is_instance_valid(_active_scene):
...
func _set_title_pulse_active(active: bool) -> void:
	if not title_label:
		return
	if active:
		if _title_pulse_running:
			return
		_title_base_scale = title_label.scale
		_title_pulse_running = true
		_start_title_pulse_loop()
	else:
		_title_pulse_running = false
		_reset_title_pulse()

func _start_title_pulse_loop() -> void:
	call_deferred("_title_pulse_loop")

func _title_pulse_loop() -> void:
	var beat_length : float = 60.0 / max(title_pulse_bpm, 1.0)
	while _title_pulse_running:
		_do_title_pulse()
		await get_tree().create_timer(beat_length).timeout

func _do_title_pulse() -> void:
	if not title_label or not title_label.is_inside_tree():
		return
	if _title_pulse_tween and _title_pulse_tween.is_running():
		_title_pulse_tween.kill()
	title_label.pivot_offset = title_label.size * 0.5
	var target_scale := _title_base_scale * title_pulse_scale
	_title_pulse_tween = create_tween()
	_title_pulse_tween.tween_property(title_label, "scale", target_scale, title_pulse_attack).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	_title_pulse_tween.tween_property(title_label, "scale", _title_base_scale, title_pulse_release).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	#if title_particles and title_pulse_particles_enabled:
	#	title_particles.position = title_label.size * 0.5
	#	title_particles.restart()
	#	title_particles.emitting = true

func _reset_title_pulse() -> void:
	if _title_pulse_tween and _title_pulse_tween.is_running():
		_title_pulse_tween.kill()
	if title_label:
		title_label.scale = _title_base_scale
	if title_particles:
